...
const STARTING_VERSE_LIMIT = 5; // Start with 5 verses unlocked
...
const initialState = {
    ...
    verseMemoryMaxVerse: STARTING_VERSE_LIMIT,
    lastPlayedDate: new Date().toISOString().split('T')[0],
    dailyStreak: 0,
    lastStreakDate: '',
    lastStreakModalShownDate: '',
}
...
interface UserProgressState {
    ...
    verseMemoryMaxVerse: number;
    lastPlayedDate: string;
    dailyStreak: number;
    lastStreakDate: string;
    lastStreakModalShownDate: string;
        ...
        (set, get) => ({
            ...
            updateStreak: () => {
                ...
                if (lastStreakDate === todayStr) {
                    return get().dailyStreak; // Already completed today
                }
                ...
                if (lastStreakDate === yesterdayStr) {
                    newStreak = get().dailyStreak + 1;
                }

                set({ dailyStreak: newStreak, lastStreakDate: todayStr });
                return newStreak;
            },
             setStreakModalShown: () => {
                const todayStr = new Date().toISOString().split('T')[0];
                set({ lastStreakModalShownDate: todayStr });
            },
            reset: () => {
                set(initialState);
            }
