...
export default function BibleMasteryPage() {
  ...
  useEffect(() => {
    ...
    let isStage2Done = false;
    if (verseMemoryProgress) {
        const progress = JSON.parse(verseMemoryProgress);
        if (progress.scores) {
            isStage2Done = isStageComplete(2, progress.scores);
        }
    }
    setIsUnlocked(isStage2Done);
  }, []);
  ...
  return (
    ...
    <div className="container mx-auto max-w-md">
        ...
        <Card id="bible-mastery-card" className="w-full">
            <CardHeader>
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                <div className="mx-auto bg-primary/10 p-4 rounded-full mb-4 w-fit">
                    <Image src="https://storage.googleapis.com/stedi-assets/bible-quest/stage-complete-badge.png" width={80} height={80} alt="Stage Complete Badge"/>
                </div>
                <AlertDialogTitle className="font-headline text-2xl text-center">
                    Level {currentLevel} Complete!
                </AlertDialogTitle>
                <AlertDialogDescription className="text-center">
                    Excellent work! You've mastered all rounds for this level. Ready for the next challenge?
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogAction onClick={proceedToNextLevel} className="w-full">
                    Continue
                </AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
