...
const sermonGuideFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (!output) {
      throw new Error("The AI failed to generate a sermon guide.");
    }

    // Add robust validation to prevent crashes from incomplete AI output.
    for (const point of output.points) {
      const result = SermonPointSchema.safeParse(point);
      if (!result.success) {
        console.error("Incomplete point generated by AI:", point, result.error);
        throw new Error("The AI generated an incomplete sermon point. Please try again.");
      }
    }
